  ...
  useEffect(() => {
    ...
    switch (live.signal.kind) {
      ...
      case 'round_result': {
        if (!inPlay) return
        /*
         * Задержка ровно на замах: руки успевают сойтись на счёт «три»,
         * и только потом открываются фигуры. Держать дольше нечего —
         * замах уже кончился, а человек ждёт следующего раунда.
         */
        const timer = setTimeout(() => go('result'), 550)
