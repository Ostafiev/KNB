 ...
 * в экран результата.
 *
 * Две с половиной секунды оказались долгими. Показать фигуры нужно, но бой
 * из трёх раундов складывался из десяти секунд игры и восьми секунд
 * разглядывания уже сыгранного: темп терялся, и между раундами хотелось
 * промотать. Полутора хватает, чтобы увидеть обе руки и счёт.
 */
const RESULT_PAUSE_MS = 1600

/**
 * Пауза перед итогами — короткая, только чтобы дать рукам договорить.
 *
 * Раньше её не было вовсе: сервер объявлял результат последнего раунда и в тот
 * же миг — конец матча. Приложение получало оба сообщения подряд и прыгало на
 * итоги прямо посреди замаха. Отсюда и ощущение, что слово «ПОБЕДА» появляется
 * «вместе с экраном итогов»: между ходом и табло не было ничего.
 *
 * Ровно полсекунды, не больше. Лишнего экрана здесь не появляется: приложение
 * открывает раскладку раунда только через 550 мс, и итоги успевают её отменить.
 * Длиннее — и в конце боя мелькнёт ещё одно окно, которого никто не просил.
 */
const FINAL_ROUND_PAUSE_MS = 500
...
export async function announceOutcome(outcome: MoveOutcome): Promise<void> {
  ...
  if (outcome.finished) {
    ...
    /*
     * Итоги приходят не сразу, а после паузы на показ решающего раунда.
     * Матч на сервере уже закрыт — деньги посчитаны и записаны; ждёт только
     * сообщение об этом. Если игрок за эти полторы секунды закроет
     * приложение, он увидит итоги при следующем входе: состояние матча
     * запрашивается заново при подключении.
     */
    const timer = setTimeout(() => {
      roundTimers.delete(outcome.match.id)
      void announceMatchFinished(outcome.match.id)
    }, FINAL_ROUND_PAUSE_MS)
    timer.unref?.()
    roundTimers.set(outcome.match.id, timer)
    return
  }
