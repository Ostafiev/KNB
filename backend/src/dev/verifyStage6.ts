...
async function main(): Promise<void> {
  ...
  try {
    ...
    check(
      /*
       * Тот же вид, что предлагается живому игроку при входе: ник или короткое
       * имя, но не имя с инициалом. По одной подписи не должно быть понятно,
       * кто перед тобой.
       *
       * Кириллица разрешена намеренно: «Макс» и «Настя» — ровно то, как
       * подписывается половина живых людей, и запрет на такую подпись выдавал
       * бы ботов с головой в обратную сторону. Запрещены пробел и точка —
       * именно по ним опознавалось паспортное «Максим Б.».
       */
      profiles.every((p) => /^[A-Za-zА-Яа-яЁё][A-Za-zА-Яа-яЁё0-9_]{1,23}$/.test(p.nickname)),
